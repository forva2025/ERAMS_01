...
- [ ] **In-app notifications** — trigger something that should notify you (e.g. a driver rejects, a call comes in) and confirm a Realtime banner/feed entry appears. This part works regardless of Firebase (it's Supabase Realtime, not push).
- [ ] **Call ringing, with real audio now** — from an active trip, tap Voice or Video Call (as in §7). Confirm the *other* party sees a **full-screen incoming-call screen** pop up (not just the existing call UI), **with the ringtone actually playing** and a vibration pattern, from anywhere in the app — not just if they happened to be on the trip-tracking/active-incident screen already. Both audio files were added 9 Sep 2026, so this should no longer be silent — if it is, that's now a real bug to report, not an expected gap.
- [ ] **OS-level push (background/killed app), Android only for now** — `flutterfire configure` ran 9 Sep 2026 so the Android app can register a real FCM token. **This still won't deliver end-to-end** until someone sets the `FCM_SERVICE_ACCOUNT_JSON` / `FCM_PROJECT_ID` Supabase Edge Function secrets (server side) — check with whoever's tracking that before spending time testing it. Web push additionally needs an `FCM_VAPID_KEY` baked in at build time, which also hasn't happened yet — don't expect this on the web app at all.

---

## 12. Known Caveats — Not Bugs

- **The driver's job-offer card has a Reject button again** (reinstated 1 Sep 2026, reversing the 29 Aug removal noted in earlier versions of this guide) — now requires picking a structured reason (see §11 above) rather than a bare reject.
- **OS-level push notifications (FCM) still don't deliver end-to-end** — Firebase itself is now configured (9 Sep 2026), but the server-side secrets (`FCM_SERVICE_ACCOUNT_JSON`/`FCM_PROJECT_ID`) that let `send_push_notification` actually call the FCM API haven't been set yet, and web additionally needs a `FCM_VAPID_KEY` baked into the build. The **in-app notification banner/feed still works regardless** — it's Supabase Realtime, not FCM.
