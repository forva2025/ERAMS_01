-- Migration 011: Patient request flow
-- Adds photo_url and pending_acceptance status to incidents.
-- Updates dispatch_incident RPC to support patient-initiated trips.
-- Adds accept_trip and decline_trip RPCs for the driver job-offer flow.
...
-- Recreate the status CHECK constraint to include pending_acceptance.
ALTER TABLE public.incidents
...
-- ── 2. Updated dispatch_incident (adds p_patient_id parameter) ─────────────────
-- Drop the old 2-param version; recreate with 3-param signature.
-- Existing Flutter calls that omit p_patient_id keep working via DEFAULT NULL.

DROP FUNCTION IF EXISTS public.dispatch_incident(uuid, uuid);

CREATE FUNCTION public.dispatch_incident(
  p_incident_id   uuid,
...
CREATE FUNCTION public.accept_trip(p_incident_id uuid)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
BEGIN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM ambulances WHERE id = v_ambulance_id AND driver_id = auth.uid()
  ) THEN
    RAISE EXCEPTION 'unauthorized';
  END IF;
...
CREATE FUNCTION public.decline_trip(p_incident_id uuid)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
BEGIN
  ...
  -- Find next nearest available ambulance, skipping the one that declined
